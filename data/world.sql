...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 25, 2024 at 09:49 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddCity` (IN `cityName` VARCHAR(50), IN `countryCode` CHAR(3), IN `district` VARCHAR(50), IN `population` INT)   BEGIN
    DECLARE countryExists INT;
    
    -- Check if the country exists
    SELECT COUNT(*) INTO countryExists FROM country WHERE Code = countryCode;
    
    IF countryExists = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Country does not exist';
    ELSE
        -- Insert city if it does not exist
        INSERT INTO city (Name, CountryCode, District, Population) VALUES (cityName, countryCode, district, population);
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddCountry` (IN `countryCode` CHAR(3), IN `countryName` VARCHAR(50), IN `continent` VARCHAR(50), IN `region` VARCHAR(50), IN `surfaceArea` DECIMAL(10,2), IN `indepYear` SMALLINT, IN `countryPopulation` INT, IN `lifeExpectancy` DECIMAL(5,2), IN `gnp` DECIMAL(10,2), IN `gnpOld` DECIMAL(10,2), IN `localName` VARCHAR(50), IN `governmentForm` VARCHAR(50), IN `headOfState` VARCHAR(50), IN `capital` INT, IN `code2` CHAR(2))   BEGIN
    DECLARE continentExists INT;
    
    -- Check if the continent exists
    SELECT COUNT(*) INTO continentExists FROM country WHERE Continent = continent;
    
    IF continentExists = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Continent does not exist';
    ELSE
        ...
        VALUES (countryCode, countryName, continent, region, surfaceArea, indepYear, countryPopulation, lifeExpectancy, gnp, gnpOld, localName, governmentForm, headOfState, capital, code2);
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddCountryLanguage` (IN `countryCode` CHAR(3), IN `language` VARCHAR(50), IN `isOfficial` BOOLEAN, IN `percentage` DECIMAL(5,2))   BEGIN
    DECLARE countryExists INT;
    ...
    IF countryExists = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Country does not exist';
    ELSE
        -- Insert country language if it does not exist
        INSERT INTO countrylanguage (CountryCode, Language, IsOfficial, Percentage) VALUES (countryCode, language, isOfficial, percentage);
    END IF;
END$$
...
CREATE TRIGGER `PreventDuplicateCity` BEFORE INSERT ON `city` FOR EACH ROW BEGIN
    ...
    -- Correctly use SELECT INTO to assign the value to a variable
    SELECT COUNT(*) INTO city_count FROM city WHERE Name = NEW.Name AND CountryCode = NEW.CountryCode;
